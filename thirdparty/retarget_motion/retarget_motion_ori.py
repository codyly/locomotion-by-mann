...
import retarget_motion.retarget_config_laikago as config
...
def build_markers(num_markers):
  # build pybullet marker object
  marker_radius = 0.02

  markers = []
  for i in range(num_markers):  # set marker color
...
def set_pose(robot, pose):
  ...
  for j in range(num_joints):
    ...
    if (j_pose_size > 0):
      ...
      pybullet.resetJointStateMultiDof(robot, j, j_pose, j_vel)  # 0-velocity
...
def process_ref_joint_pos_data(joint_pos):  # pos of single frame
...
def retarget_root_pose(ref_joint_pos):
  ...
  root_pos = 0.5 * (pelvis_pos + neck_pos)  # root pos selected usingi heuristic
...
def retarget_pose(robot, default_pose, ref_joint_pos):  # real thing going on!
...
def retarget_motion(robot, joint_pos_data):
  ...
  for f in range(num_frames):
    ...
    ref_joint_pos = np.reshape(ref_joint_pos, [-1, POS_SIZE])  # (81,) -> (27,3)
    ref_joint_pos = process_ref_joint_pos_data(ref_joint_pos)  # coordinate transform reference motion
...
def main(argv):
  ...
  while True:
    ...
    for mocap_motion in mocap_motions:
      ...
      retarget_frames = retarget_motion(robot, joint_pos_data)  # main part done here! Retarget whole sequence
      output_motion(retarget_frames, f"{mocap_motion[0]}.txt")
    
      f = 0
      num_frames = joint_pos_data.shape[0]
    
      for repeat in range (5*num_frames):  # Visualization only step here! repeat 5 times for WHAT reason?
        ...
        # update_camera(robot)
        p.configureDebugVisualizer(p.COV_ENABLE_SINGLE_STEP_RENDERING,1)  # rendering step
        f += 1
